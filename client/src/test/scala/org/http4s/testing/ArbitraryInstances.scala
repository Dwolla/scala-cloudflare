/* This is copied from http4s 0.18 instead of pulling in http4s-testing because the binary
 * distribution relies on a binary-incompatible version of scalacheck. Once http4s is upgraded,
 * this file can be removed and we can rely on the published version instead.
 */
